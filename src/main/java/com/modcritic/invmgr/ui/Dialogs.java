...
/**
 * The bits every dialog is made of: the box, its title, its rows of labeled fields, and its
 * buttons.
 *
...
public final class Dialogs {
    ...
    public enum ButtonKind {
        /** Cancel, and the swap button: plain gray. */
        PLAIN,
    ...
    /** The dialog's panel: dark gray, square, with a one-pixel border. */
    public static VBox box() {
        VBox box = new VBox();
        box.setPadding(new Insets(TouchType.dialogPadding()));
        box.setMinWidth(TouchType.dialogMinWidth());
    ...
    public static Label title(String text) {
        ...
        label.setFont(Font.font(Tokens.FONT_FAMILY, TouchType.dialogTitleFont()));
    ...
    /** The gray caption to the left of a field. */
    public static Label fieldLabel(String text) {
        Label label = new Label(text);
        label.setFont(Font.font(Tokens.FONT_FAMILY, TouchType.dialogFont()));
        label.setTextFill(Tokens.TEXT_DIALOG_LABEL);
        return label;
    }

    /**
     * One labeled row: caption on the left, field hard against the right edge.
     *
    ...
    public static TextField textInput(double width, double fontSize, int maxLength) {
        ...
        field.setStyle("-fx-background-color: " + Tokens.hex(Tokens.DIALOG_INPUT_BG) + ";"
                ...
                + "-fx-border-width: 1;"
                + "-fx-background-radius: 0; -fx-border-radius: 0;"
                + "-fx-padding: " + TouchType.dialogInputPaddingV() + " "
                + TouchType.dialogInputPaddingH() + " " + TouchType.dialogInputPaddingV() + " "
                + TouchType.dialogInputPaddingH() + ";");
    ...
    public static HBox idField(TextField field) {
        ...
        label.setFont(Font.font(Tokens.FONT_FAMILY, TouchType.idLabelFont()));
    ...
    /**
     * The right-aligned strip of buttons along the bottom of a dialog, <b>which wraps onto a second
     * line rather than squeezing when it runs out of room.</b>
     *
     * <p>A {@code FlowPane} and not an {@code HBox}, because the original is
     * {@code .modal-btns { display: flex; flex-wrap: wrap }} and the wrap is load-bearing on a
     * phone. The Edit dialog carries Delete, the ID field, Cancel and Confirm (about 396 px of
     * content inside a 360 px screen), so in the original it simply takes two lines.
     *
     * <p><b>An HBox does not do that; it shrinks its children instead, and a shrunk JavaFX button
     * ellipsizes its own label.</b> That is where the user's {@code C...} came from: not from the
     * text being wrong, but from a row that had no way to say "I need another line". A browser
     * button never truncates its label; it grows the box, and the flex container wraps.
     */
    public static FlowPane buttonRow(Node... children) {
        FlowPane row = new FlowPane(Orientation.HORIZONTAL,
                Tokens.DIALOG_BUTTON_GAP, Tokens.DIALOG_BUTTON_GAP, children);
     ...
     * A dialog button.
     *
     * <p>Hovering lightens a plain button and deepens a colored one, exactly as the original's
    ...
    public static Button button(String text, ButtonKind kind) {
        Button button = button(text, kind,
                TouchType.dialogButtonPaddingV(), TouchType.dialogButtonPaddingH());
        button.setMinHeight(TouchType.dialogButtonHeight());
        button.setPrefHeight(TouchType.dialogButtonHeight());
        button.setMaxHeight(TouchType.dialogButtonHeight());
        return button;
    }
    ...
    private static Button button(String text, ButtonKind kind, double paddingV, double paddingH) {
        ...
        button.setFont(Font.font(Tokens.FONT_FAMILY, TouchType.dialogButtonFont()));
        // A button may never ellipsize its own label. A browser's never does (it grows the box and
        // lets the flex row wrap), and JavaFX's default is the opposite, which is what turned
        // Cancel into "C..." on the phone. Tying the minimum to the preferred width takes the
        // choice away: the row wraps (see buttonRow) instead of the words disappearing.
     ...
     *       centered on its own ink rather than on the math face's very tall line box.
     ...
     *       46 × 30, because the width followed the glyph's advance, and a math face's advances
     ...
     * symmetric. Handing the whole square to the graphic makes the centering mean something.
     *
    ...
    public static Button symbolButton(String text, ButtonKind kind, double size) {
        ...
        button.setText(null);
        button.setGraphic(Fonts.symbolGlyph(text, TouchType.dialogFont(), button));
