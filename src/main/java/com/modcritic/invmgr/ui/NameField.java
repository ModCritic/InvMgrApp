...
public final class NameField extends TextArea {
    ...
    /**
     * The name box never gets narrower than a plain text input in the same dialog.
     *
     * <p>A method rather than a constant, and that is not fussiness: a static final would read the
     * platform switch once, at class-load, and keep that answer for the life of the JVM. The whole
     * test suite shares one JVM and flips that switch per class.
     */
    private static double minWidth() {
        return TouchType.dialogTextWidth();
    }
    ...
    public NameField(double fontSize) {
        ...
        setPadding(new Insets(TouchType.dialogInputPaddingV(), TouchType.dialogInputPaddingH(),
    ...
    public void resizeToFit() {
        ...
        double verticalChrome = TouchType.dialogInputPaddingV() * 2;

        double widest = widestLine();
        double width = Math.max(minWidth(), Math.min(widthCap(), widest + horizontalChrome));
        setPrefWidth(width);
        setMinWidth(width);
        setMaxWidth(width);
    ...
    /** The widest single line of the current text, or of the placeholder, when there is none. */
    private double widestLine() {
        String text = getText();
        if (text == null || text.isEmpty()) {
            // An empty box is as wide as its PLACEHOLDER, not as wide as nothing. The original sets
            // `field-sizing: content` on this textarea, and in CSS a placeholder counts towards a
            // field's intrinsic content size, which is why the original opens showing "Leave blank
            // for default" in full where this opened with it cut off at "Leave blank for".
            //
            // Measured on a 360 px phone: the original's box is about 195 px, which is the 179 px
            // that sentence needs plus its padding and border, comfortably past the 160 px
            // `min-inline-size` the same rule sets. Returning zero here pinned it to that minimum.
            String placeholder = getPromptText();
            return placeholder == null || placeholder.isEmpty() ? 0 : measure(placeholder);
        }
        double widest = 0;
